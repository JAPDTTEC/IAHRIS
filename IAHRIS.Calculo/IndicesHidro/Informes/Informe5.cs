...
{
    public class Informe5 : Informe
    {
        public Informe5(bool active, int index, string nombreInformeXML, string nombrePestaña) : base(active, index, nombreInformeXML, nombrePestaña)
        {
        }
        ...
        public override void Escribir(ExcelPackage excel, ref DatosCalculo datos,  ref IAHRISDataSet dataset, MultiIdiomasXML traductor)
        {
            ...
            for (int i = 0; i <= 3; i++)
            {
                objSheet.Cells[pos + 1, offsetCol + 1].Value = dataset._HabMagnitudAlt[i];
                pos++;
            }

            for (int i = 0; i <= 3; i++)
            {
                objSheet.Cells[pos + 1, offsetCol + 1].Value = dataset._HabVariabilidadAlt[i];
                pos++;
            }

            for (int i = 0; i <= 2; i++)
            {
                objSheet.Cells[pos + 1, offsetCol + 1].Value = dataset._HabEstacionalidadAlt[i];
                pos++;
            }
