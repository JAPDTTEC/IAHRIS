...
{
    public class Informe6d : Informe
    {
        public Informe6d(bool active, int index, string nombreInformeXML, string nombrePestaña) : base(active, index, nombreInformeXML, nombrePestaña)
        {
        }
        ...
        public override void Escribir(ExcelPackage excel, ref DatosCalculo datos,  ref IAHRISDataSet dataset, MultiIdiomasXML traductor)
        {
            ...
            calculations.CalcularCurvaMensualClasificadaCudales(true);
                ...
                for (int k = 0; k < 31; k++)
                {
                    try
                    {
                        objSheet.Cells[offsetRow + k + 1, offsetCol + mpos + 1].Value =
                            dataset.CurvaClasificadaMensualCaudalAlterada.SerieEstructurada[m].SerieClasificada.ValoresK[k].Media;
                    }
