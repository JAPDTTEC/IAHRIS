...
{
    public class Informe5b : Informe
    {
        public Informe5b(bool active, int index, string nombreInformeXML, string nombrePestaña) : base(active, index, nombreInformeXML, nombrePestaña)
        {
        }
        ...
        public override void Escribir(ExcelPackage excel, ref DatosCalculo datos,  ref IAHRISDataSet dataset, MultiIdiomasXML traductor)
        {
            ...
            calculations.CalculoParametrosAvenidasSoloAlteradosCASO6();
            calculations.CalculoParametrosSequiasSoloAlteradosCASO6();
            ...
            for (int i = 0; i <= 3; i++)
            {
                objSheet.Cells[offsetRow + 1, offsetCol + 1].Value = dataset._HabMagnitudAlt[i];
                offsetRow++;
            }
            
            for (int i = 0; i <= 3; i++)
            {
                objSheet.Cells[offsetRow + 1, offsetCol + 1].Value = dataset._HabVariabilidadAlt[i];
                offsetRow++;
            }
            ...
            for (int i = 0; i <= 2; i++)
            {
                objSheet.Cells[offsetRow + 1, offsetCol + 1].Value = dataset._HabEstacionalidadAlt[i];
                offsetRow++;
            }
            ...
            // range(2, 1) = Me.dataset._AveMagnitudAlt(1)
