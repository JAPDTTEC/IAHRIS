...
namespace IAHRIS
{
    public partial class FormEliminarProyecto
    {
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            ...
            DialogResult result = MessageBox.Show(_traductor.traducirMensaje(MultiLangXML.MultiIdiomasXML.TIPO_MENSAJE.M_INFO, "strDeleteProyectAdv"),
                    "Atención", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
            if (result == DialogResult.OK)
            {
                Enabled = false;

                ComboItem itemSeleccionado = (ComboItem)cbProyectos.SelectedItem;
                int filas = _cMDB.EjecutarSQL(Conversions.ToString(Operators.ConcatenateObject("DELETE FROM [Proyecto] WHERE ID_proyecto=", itemSeleccionado.Id)));
