...
{
    /// <summary>
    /// Formulario que se usa para el calculo y generación de Informes de Excel.
    /// </summary>
    public partial class FormCalculo
    {
        ...
        private void btnCancelar_Click(object sender, EventArgs e, string ruta=null)
        {
            ...
            _reportController.GenerarInforme(tipologia, _datos, ruta);
        ...
        private void FormCalculo_Load(object sender, EventArgs e)
        {
                ...
                if (_simulacion.añosParaCalculo[2].nAños > 0)
                {
                    ...
                    if (pos >= 0)
                    {
                        ...
                        if (necesitaInterpolar & posInt >= 0)
                        {
                            ...
                            int añoAux = año;
                            int mesfinal = datos.mesInicio - 1;
                            int añofinal = año + 1;
                            ...
                            for (int i = 0; i < 12; i++)
                            {
                                ...
                                datos.SerieNatMensual.caudalMensual[datos.SerieNatMensual.caudalMensual.Length - 1] = Utiles.InterpolarMes(añoAux, mes, _simulacion.idListas[0], _cMDB);
                                datos.SerieNatMensual.mes[datos.SerieNatMensual.mes.Length - 1] = new DateTime(añoAux, mes, 1);

                                mes++;
                            }

                            //Añadir año natural al terminar los meses.
                    ...
                    if (pos >= 0)
                    {
                        ...
                        if (necesitaInterpolar & posInt >= 0)
                        {
                            ...
                            for (int i = 0; i < 12; i++)
                            {
                                ...
                                datos.SerieAltMensual.caudalMensual[datos.SerieAltMensual.caudalMensual.Length - 1] = Utiles.InterpolarMes(añoAux, mes, _simulacion.idListas[1], _cMDB);
            ...
            if(_simulacion.Tipologia.Nombre == enumTipologias.Tipo6B.ToString() ||
                _simulacion.Tipologia.Nombre == enumTipologias.Tipo8B.ToString() || 
                _simulacion.Tipologia.Nombre == enumTipologias.Tipo8BSR.ToString())
            {
        ...
        private void FormCalculo_Shown(object sender, EventArgs e)
        {
                ...
                foreach (string arg in argsList)
                    {
                        if (arg.StartsWith("/"))
                        {
                            ...
                            {
                                throw new Exception("Parametro/s incorrecto/s");
                            }
                            else
                            {
            ...
            if (parametros.ContainsKey("/fs"))
            {

                Application.OpenForms["FormInicial"].Hide();
                My.MyProject.Application.DoEvents();
                
                parametros.TryGetValue("/fs", out string ruta);
                btnCancelar_Click(null, null, ruta);
