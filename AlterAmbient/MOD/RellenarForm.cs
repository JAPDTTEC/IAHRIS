...
namespace IAHRIS.Rellenar
{
    class RellenarForm
    {
        ...
        public double[][] ObtenerValoresPorMesesyAñoFromLista(int idLista) 
        {
            string consulta = string.Format("SELECT Year(FECHA) AS AÑO,  Month(FECHA) AS MES, Sum(VALOR.VALOR) AS VALOR_MES FROM VALOR WHERE ID_LISTA = {0} GROUP BY Month(FECHA), Year(FECHA), ID_LISTA ORDER BY Year(FECHA);", idLista);
            DataSet dsValores = _cMDB.RellenarDataSet("dsValores", consulta);

            if (dsValores == null)
                return null;

            int numLineas = dsValores.Tables[0].Rows.Count;
            double[][] valores = new double[numLineas][];
            int numFilaAct = 0;


            foreach (DataRow fila in dsValores.Tables[0].Rows)
            {
                double[] filaValor = new double[3]
                {
                    Convert.ToDouble(fila["AÑO"]),
                    Convert.ToDouble(fila["MES"]),
                    Convert.ToDouble(fila["VALOR_MES"])
                };
                valores[numFilaAct] = filaValor;
                numFilaAct++;
            }
        ...
        public TestFechas.Simulacion RellenarXPTable(ref Table tabla, int idalt, int idpunto, bool usarCoe, bool usarCoeDiaria)
        {
            ...
            TestFechas.Coetaneidad coeD = default;
            TestFechas.Coetaneidad coeM = default;
            ...
            coeD = _Fechas.CalcularCoetaneidad(validosND, validosAD, null, null);
            coeM = _Fechas.CalcularCoetaneidad(validosNM, validosAM, añosInterNat, añosInterAlt);
            ...
            for (i = añoMin; i <= loopTo4; i++)
            {
                ...
                if (validosNM.nValidos + nAñosIntN >= 7)
                {
                    if (validosND.nValidos >= 7)
                    {
                        if (usarCoeDiaria)
                        {
                            if (cells[10].Text == "CO")
                    ...
                    if (usarCoe)
                    {
                        if (cells[5].Text == "CO")
                        {
                            ...
                            if (validosAM.nValidos + nAñosIntA >= 7)
                            {
                ...
                else if (validosAM.nValidos + nAñosIntA >= 7)
                {
                    if (cells[6].Text == "C")
                    {
                        cells[7].Text = "";
            ...
            var c = new TestFechas.Coetaneidad[] { coeD, coeM };
        ...
        public int RellenarProyectosNomb(ref Label label, int id)
        {
            if (id == -1)
            {
                return -1;
        ...
        public int RellenarPersRegPunto(ref Label label1, ref Label label2, int id)
        {
            ...
            var ds = _cMDB.RellenarDataSet("Punto", "SELECT nombreRegimen, abreviaturaRegimen, ID_Punto FROM [Punto] WHERE ID_Punto = " + id);
            var dr = ds.Tables[0].Rows[0];
            if (label1 is object && label2 is object)
            {
                label1.Text = dr[0].ToString();
                label2.Text = dr[1].ToString();
            }

            return Conversions.ToInteger(dr[2]);
        }
        public int RellenarPersRegAlteracion(ref Label label1, ref Label label2, int id)
        {
            ...
            var ds = _cMDB.RellenarDataSet("Alteracion", "SELECT nombreRegimen, abreviaturaRegimen, ID_Alteracion FROM [Alteracion] WHERE ID_Alteracion = " + id);
            var dr = ds.Tables[0].Rows[0];
            if (label1 is object && label2 is object)
            {
                label1.Text = dr[0].ToString();
